...
import { Users, UserTypes, Recipes } from "./src/models/schema.js";
import session from "express-session";
import { importRecipes } from "./src/scripts/importRecipes.js";
import { addUser, importUserTypes } from "./src/scripts/importUser.js";
...
app.get("/", async (req: Request, res: Response) => {
  try {
    const allRecipes = await Recipes.find({}).lean();
  ...
  // By mapping the sorted data in the object to each iteration of the method, I was able to produce an html list including all the featured recipes.
    const listHTML = featuredRecipes.map(recipe => 
      `<a class="recipe-card-link" href="/recipe/${recipe.id}">
...
app.get("/search", async (req: Request, res: Response) => {
  try {
    ...
    const order = req.query.order as string || "alphabet";
    ...
    const dbRecipes = await Recipes.find(filter).sort(sort).lean();

  //html is created and sent as a string to be displayed
  const listHTML = dbRecipes.map(recipe => 
      `<a class="recipe-card-link" href="/recipe/${recipe.id}">
                <section class="recipe-card">
                    <img class="recipe-card-img" src="/images/recipes/${recipe.id}.jpg" alt="${recipe.title} image">
                    <h3 class="recipe-card-title">${recipe.title}</h3>
...
app.get("/recipe/:id", async (req: Request, res: Response) => {
  ...
  try {
    const recipe = await Recipes.findOne({ id: recipeId }).lean();
    if (!recipe) {
...
app.get("/reset-db", (req: Request, res: Response) => {
  if ((req.session as any).user) {
    importRecipes();
    res.redirect("/");
  }
...
app.post("/new-recipe", async (req: Request, res: Response) => {
  try {
    const { id, title, description, image, upload_date, categories, ingredients, instructions } = req.body;
    const newRecipe = new Recipes({id, title, description, image, upload_date, categories, ingredients, instructions});
...
app.post("/add-user", async (req: Request, res: Response) => {
  try {
    ...
    const newUser = new Users({fname, lname, email, password, token, user_type,});
